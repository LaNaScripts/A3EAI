...
if (_configExists) then {
	...
	if (A3EAI_debugLevel > 0) then {diag_log format ["A3EAI Debug: Spawning in serverside objects... Reading from file %1.",_generatorStr];};

	for "_i" from 0 to ((count _cfgLocation) - 1) do {
		//_locationArray set [count _locationArray,configName (_cfgLocation select _i)];
		_locationArray pushBack (configName (_cfgLocation select _i));
	};
	...
	_spawnServerObj = {
		...
		for "_i" from ((count _this) - 1) to 0 step -1 do {
			...
			if (isClass (_config)) then {
				...
				if ((_i % 25) isEqualTo 0) then {uiSleep 0.01;};
			};
		};
	};
	...
	if (A3EAI_debugLevel > 0) then {diag_log format ["A3EAI Debug: Serverside object patch completed in %1 seconds.",(diag_tickTime - _startTime)]};
} else {
	if (A3EAI_debugLevel > 0) then {diag_log "A3EAI Debug: CfgTownGenerator not found. Aborting serverside object patch. (This is not an error)";};
};
