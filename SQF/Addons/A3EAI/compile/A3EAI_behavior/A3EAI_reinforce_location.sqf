...
if ((!isNull _trigger) && {_trigger in A3EAI_dynTriggerArray}) then {
	...
	if ((!isNil "_targetPlayer") && {!(isNull _targetPlayer)}) then {
		...
		if ((_targetPlayerVehicle distance _trigger) < 300) then {
			...
			if (({"EpochRadio0" in (assignedItems _x)} count _targetPlayerVehicleCrew) > 0) then {
				_radioText = format ["Warning: Enemy %1 inbound.",[configFile >> "CfgVehicles" >> (typeOf _helicopter),"textSingular","helicopter"] call BIS_fnc_returnConfigEntry];
				{
